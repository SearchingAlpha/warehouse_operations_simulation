"""
Cross-dock simulation using a deliberately simple SimPy-based discrete-event model.
The goal is directional insight (throughput, queues, bottlenecks), not high fidelity.

Simplifications called out explicitly:
- Ignore travel distance, breaks, and detailed labor rules.
- Service times are exponential by default with a configurable mean; set mean to zero
  for an effectively instantaneous step.
- Arrivals can be Poisson or a simple scheduled piecewise constant rate.
- Conveyor is modeled as a server with a service-time approximation of 1/rate.
- Lanes have optional finite queue capacity; arrivals block (not drop) when full.

Outputs include throughput, queue sizes, WIP, utilization, lead time distribution, and a
naive bottleneck heuristic (highest utilization or growing queues).

Running the module executes three scenarios: baseline, stress (heavier arrivals), and a
bottleneck fix (more sort labor). A small parameter sweep helper is also provided.
"""
...
def build_baseline_config() -> SimulationConfig:
    return SimulationConfig(
        sim_duration_min=480,
        warmup_min=60,
        arrival_type="poisson",
        mean_arrival_rate_per_min=12,
        n_routes=6,
        route_probabilities=[0.15, 0.2, 0.15, 0.2, 0.2, 0.1],
        n_inbound_workers=3,
        inbound_service_time_mean=0.6,
        conveyor_rate_items_per_min=45,
        n_sort_workers=4,
        sort_service_time_mean=0.7,
        per_route_n_load_workers=1,
        load_service_time_mean=1.8,
        per_route_queue_cap=50,
        random_seed=7,
    )
...
def main():
    ...
    base_results = run_scenario(baseline, "Baseline")

    # Stress: heavier arrivals
    stress = copy.deepcopy(baseline)
    stress.mean_arrival_rate_per_min = 18
    run_scenario(stress, "Stress (higher arrivals)")

    # Bottleneck fix: add sort labor if sort appears constrained
    fix = copy.deepcopy(stress)
    fix.n_sort_workers = 6
    run_scenario(fix, "Fix (more sort labor)")

    # Parameter sweep for quick what-if comparisons
    run_parameter_sweep(
        baseline,
